...
ComplyScan v0.8 uses Vercel for the Next.js frontend and a separate Python host for the FastAPI OCR backend.
...
```text
COMPLYSCAN_ALLOWED_ORIGINS=https://<your-vercel-domain>
```
...
The current prototype accepts images larger than Vercel's normal function request-body limit. Keeping uploads direct from the browser to the Python backend preserves the existing ComplyScan upload contract and avoids an unnecessary proxy hop.
